...
class GameBody extends Component {
  ...
  checkIfWin() {
    ...
    for (var i = 0; i < winSegments.length; i++) {
      for (var j = 0; j < segments.length; j++) {
        if (
          winSegments[i].point.x === segments[j].point.x &&
          winSegments[i].point.y === segments[j].point.y
        ) {
          found = true
          break
        } else {
  ...
  removeLastSegment() {
    ...
    this.state.x = this.state.path.segments[
      this.state.path.segments.length - 1
    ].point.x
    this.state.y = this.state.path.segments[
      this.state.path.segments.length - 1
    ].point.y
    this.props.usedSegsFunc(this.state.path.segments.length)
  }
  ...
  Compare(path) {
    if (path.segments.length <= 1) {
      ...
      if (
        segment1.point.x < segment2.point.x &&
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.reset !== this.props.reset) {
      ...
      if (this.state.clickedPlay === true) {
        this.props.createAlertFunc(
          "You are playing, just click any of the arrow to start drawing",
          "4"
        )
      } else {
        this.resetCanvas()
